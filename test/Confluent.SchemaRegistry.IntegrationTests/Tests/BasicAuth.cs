    ...
    public static partial class Tests
    {
        ...
        public static void BasicAuth(Config config)
        {
            ...
            using (var sr = new CachedSchemaRegistryClient(
                new Dictionary<string, object>
                { 
                    // assume protocol isn't specified in url.
                    { "schema.registry.url", $"{config.Username}:{config.Password}@{config.ServerWithAuth}" }
                }))
            {
                var topicName = Guid.NewGuid().ToString();
                var subject = sr.ConstructValueSubjectName(topicName);
            ...
            Assert.Throws<ArgumentException>(() => { var sr = new CachedSchemaRegistryClient(new Dictionary<string, object>
                { 
                    { "schema.registry.url", config.ServerWithAuth },
                    { "schema.registry.basic.auth.credentials.source", "SASL_INHERIT" },
                    { "schema.registry.basic.auth.user.info", $"{config.Username:config.Password}" }
            ...
            // connect to non-authenticating. ok, not?
