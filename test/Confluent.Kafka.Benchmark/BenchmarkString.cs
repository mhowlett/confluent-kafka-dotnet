...
{
    public static class BenchmarkString
    {
        public static void ProduceConsume(string bootstrapServers, string topic, int messageCount)
        {
            var autoEvent = new AutoResetEvent(false);
            var counter = messageCount;
            Action<DeliveryReport<Null, string>> dh = (DeliveryReport<Null, string> deliveryReport) 
                => { if (--counter == 0) { autoEvent.Set(); } };

            var toProduce = "0123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789";

            DeliveryResult<Null, string> firstProduced;
            using (var producer = new Producer<Null, string>(
                Configuration.GetProducerConfig(bootstrapServers), Serializers.Null, Serializers.Utf8))
            {
                // produce a message before starting the timer to avoid including any warmup time in result.
                firstProduced = producer.ProduceAsync(topic, new Message<Null, string> { 
                    Value = toProduce }).Result;

                var startTime = DateTime.Now.Ticks;
                for (int i=0; i<messageCount; ++i)
                {
                    producer.BeginProduce(topic, new Message<Null, string> { Value = toProduce }, dh);
                }
                autoEvent.WaitOne();
            ...
            using (var consumer = new Consumer<Null, string>(
                Configuration.GetConsumerConfig(bootstrapServers), Deserializers.Null, Deserializers.Utf8))
            {
